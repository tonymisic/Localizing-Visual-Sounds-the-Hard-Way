...
from datasets import PerFrameLabels
import cv2
            ...
            for j in range(spec.shape[0]):
                ...
                if False:
                    if (i % 20 == 0 and i != 0): # labeled frames to calculate cIoU
                        ...
                        #Image.fromarray(np.uint8(np.add((image_now[0].cpu().numpy() * 255).transpose((1,2,0)) * 0.5, temp * 0.5))).convert('RGB').save("imgs/" + name[0].strip('.mp4') + "_gts/gt" + str(i) + ".jpg")
                        evaluator = Evaluator()
